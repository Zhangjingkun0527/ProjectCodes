...
public class VideoJobController {
	...
	@ResponseBody
	public String uploadPic(String data1,String user,String video,HttpServletRequest request) {
		try {
			String serverPath = request.getSession().getServletContext().getRealPath("/");
	        Base64 base64 = new Base64();
	        byte[] k = base64.decode(data1.substring("data:image/png;base64," .length()));  
            InputStream is = new ByteArrayInputStream(k);
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss");
            String fileName = simpleDateFormat.format(new Date());  
            fileName=user+"_"+video+"_"+fileName;
	...
	public ModelAndView postLoginPage(HttpServletRequest request,
		...
		try {
			...
			String sectionId = request.getParameter("sectionid");
			if(!sectionId.equals("") && sectionId!=null){
	...
	public String  clickpredicttime(String userid, String happentime, String clicktime, HttpServletRequest request,
			HttpServletResponse response) {
		
		try {
			logger.debug("click_predict_time start ------>****************************");
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
			Date click_time=new Date();
			click_time=df.parse((String)request.getParameter("clicktime"));
			//PredictScoreIndexInfo predictscoreIndexInfo=this.predictscoreIndexInfoMapper.selectByPredicttime();
			PredictScoreInfo predictScoreInfo = new PredictScoreInfo();
			predictScoreInfo.setUserid(request.getParameter("userid") );
			predictScoreInfo.setClicktime(click_time );
			//predictScoreInfo.setHappentime(happen_time);
			
			String result=iVideoJobService.predictClickRecord(predictScoreInfo);
			
			//logger.debug(result);
			logger.debug(">>>click_predict_time_time END<<<***********");
			//return "success";
		}catch (Exception e) {
			logger.error(String.format("查看预测时间请求失败"), e);
			return "failure";
		}
		return "success";
	}
	...
	public RestModel  videoStarRating(HttpServletRequest request,
		...
		try {
			...
			//starRatingInfo.setUserid( "1701210871" );
			//starRatingInfo.setSectionid( "beidawlf_01_02_02" );
