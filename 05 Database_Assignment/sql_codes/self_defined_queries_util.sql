...
BEGIN
	...
	, ' VALUES (\'', insert_value, '\')');
	PREPARE insert_statement_pre FROM @insert_statement;
	EXECUTE insert_statement_pre;
END //
DELIMITER ;
...
BEGIN
	SET @update_statement = CONCAT('UPDATE ', target_database, '.',target_table,' SET ', target_column_name, ' = \'', update_value, '\' WHERE id = ', id);
...
BEGIN
	...
	DECLARE origin_column_name VARCHAR(255);
